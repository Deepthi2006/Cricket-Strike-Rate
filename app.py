...
def predict():
    ...
    Age = float(request.form.get('t1'))
    hundred= float(request.form.get('t1'))
    fifty = float(request.form.get('t1'))
    sixty = float(request.form.get('t1'))

    # Convert the input to the appropriate format for the model
    # (Adjust preprocessing as per your model's requirements)
    input_data = [[Age,hundred,fifty,sixty]]  # Example: single numeric input
